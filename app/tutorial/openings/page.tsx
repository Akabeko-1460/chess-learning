      ...
      <div className="mb-12">
        <div className="mb-4 flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#EDF2F7] text-2xl text-navy-light">
            <i className={openingsModule.icon} />
          </div>
          <h1 className="font-serif text-3xl text-navy md:text-4xl">{openingsModule.title}</h1>
        </div>
        <p className="text-lg text-text-secondary">{openingsModule.description}</p>
      </div>
      <div className="mx-auto grid max-w-4xl grid-cols-1 gap-4 md:grid-cols-2">
        {openingsModule.lessons.map((lesson, i) => (
          <Link
            key={lesson.slug}
            href={`/tutorial/openings/${lesson.slug}`}
