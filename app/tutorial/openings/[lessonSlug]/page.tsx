...
export default function OpeningsLessonPage({
  ...
  const lesson = openingsModule.lessons.find((l) => l.slug === lessonSlug);
  if (!lesson) notFound();

  const lessonIndex = openingsModule.lessons.findIndex(
    (l) => l.slug === lessonSlug,
  );
  const nextLesson = openingsModule.lessons[lessonIndex + 1];
      ...
      <div className="mx-auto flex max-w-5xl flex-col items-start gap-8 lg:flex-row">
        ...
        <div className="flex w-full flex-col gap-4 lg:w-96">
          ...
          <LessonNavigation
            ...
            nextLessonHref={
              nextLesson ? `/tutorial/openings/${nextLesson.slug}` : undefined
            }
          />
        </div>
      </div>
